...
for i in range (n):
    ...
    if( lngratio > 4.1): # odległość jest wystarczająco duża
        cv2.line(image2, start, end, [0,230, 230], 2)
        cv2.circle(image2, far, 5, [0, 0, 255], -1)
        l = l +1
        if (ang < 1.4): # kąt ostry 
            lo = lo + 1
        if (ang > 1.4 and ang < 1.65): # kąt prosty
            rangle = 1
...
if lo == 0:
    if w > 1.09 and w < 1.27 and lk < 7 and aq < 0.8:
        cv2.putText(image2,'Wskazywanie',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lo == 1 or lo == 0:
    if lk > 7 and lk < 10:
        if aq > 0.5 and aq < 0.62:
            if w > 1.27:
                cv2.putText(image2,'Legiunia',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lo == 1:
    if l == 2 or l==1:
        if w > 1.21 and w < 1.35:
            if lk == 6 or lk == 7:
                cv2.putText(image2,'Dwa',(50,1000), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lo == 2:
    if lk == 9 or lk == 8:
        if l==3 or l == 4 or l==2:
            if w < 1.4 and w > 1.3:
                cv2.putText(image2,'Trzy',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lo == 3:
    if lk == 9 or lk == 10 or lk == 11:
        if w > 1.4 and w < 1.6:
            cv2.putText(image2,'Cztery',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lo ==4 and lk > 1.8:
    if aq > 0.59 and aq < 0.78:
        if lk > 8:
            cv2.putText(image2,'Machanie',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
    if aq > 0.78:
            cv2.putText(image2,'Piatka',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if aq < 0.58 and lk > 11 and w > 1.6:
    cv2.putText(image2,'Machanie',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lk ==7 and lo == 1:
    if w > 1.28 and w < 1.4:
        cv2.putText(image2,'Przekazany znak pokoju',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if rangle == 1:
    cv2.putText(image2,'OK',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if lo == 0 and l ==0:
    if aq > 0.75:
        cv2.putText(image2,'Piesc',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
if l == 1:
    if lo == 1 or lo == 0:
        if lk == 6 or lk == 5:
            cv2.putText(image2,'Znak szatana',(50,100), font, 2, (0,0,255), 3, cv2.LINE_AA)
